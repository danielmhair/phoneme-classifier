...
- `embeddings_cache.py` - extracts + caches temporal embeddings once per base model (Wav2Vec2 shared by MLP and Wav2Vec2-CTC; WavLM for WavLM-CTC), resumable, deterministic (no extraction-time augmentation), cacheable across folds. Index writes are atomic (temp file + `os.replace`) - a process kill mid-write previously left `index.json` as null bytes on NTFS, breaking resume (recovered by rebuilding the index from the on-disk `.npy` files).
- `augmentation.py` - optional training-only augmentation (`run_loso(use_augmentation=True)`): reuses the exact `s0b_augment_audio.py` transforms (speed 0.9x/1.1x, pitch ±2 semitones, noise overlay), seeded per-file for determinism/resumability, cached under `evaluation/harness_cache/augmented_recordings/<speaker>/<phoneme>/`. Augmented rows are only ever added to training sets, never test sets. Used by the 2026-07-11 augmented Chloe-fold pilot (see the PRD).
