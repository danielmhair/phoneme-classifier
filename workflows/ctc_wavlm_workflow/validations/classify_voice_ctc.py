"""
WavLM CTC Voice Classification Validation

This script provides real-time phoneme sequence classification using the trained WavLM CTC model.
It supports both live audio recording and file-based inference, with CTC decoding
for variable-length phoneme sequence prediction using WavLM speech features.

WavLM provides superior speech representation compared to Wav2Vec2 for phoneme recognition.
"""
...
try:
    ...
    print("⚠️ PyTorch/Transformers not available. WavLM CTC validation will use mock mode.")
...
class MockCTCClassifier:
    """Mock WavLM CTC classifier for testing when dependencies are unavailable."""
    
    def __init__(self, phoneme_labels):
        self.phoneme_labels = phoneme_labels
        self.num_classes = len(phoneme_labels)
        print(f"🤖 Mock WavLM CTC classifier initialized with {self.num_classes} phoneme classes")
...
class CTCPhonemeClassifier:
    """WavLM CTC-based phoneme sequence classifier."""
    
    def __init__(self, model_path: str, phoneme_labels_path: str, device: str = 'cpu'):
        """
        Initialize WavLM CTC classifier.
        
        Args:
            model_path: Path to trained WavLM CTC model
        ...
        if TORCH_AVAILABLE and CTC_MODEL_AVAILABLE:
            ...
            # Load WavLM processor (uses Wav2Vec2Processor)
            ...
            self.processor = None
            print("🤖 Using mock WavLM CTC classifier")
    
    def preprocess_audio(self, audio_data: np.ndarray) -> np.ndarray:
        """
        Preprocess audio for WavLM CTC inference.
...
def interactive_mode(classifier: CTCPhonemeClassifier):
    ...
    """
    print("\n🎙️ Interactive WavLM CTC Phoneme Sequence Classification")
    print("Commands:")
...
def main():
    """Main function for WavLM CTC voice classification validation."""
    parser = argparse.ArgumentParser(description="WavLM CTC Phoneme Sequence Classification")
    ...
    print("🚀 Initializing WavLM CTC Phoneme Classifier...")
