...
**Status:** Core deliverables COMPLETE - declared by Daniel 2026-07-12: the "harness done" bar (§6) and the live-mic smoke test (§5.4) are met. The 85% ship bar on Chloe is NOT met by any model (best honest number: WavLM-CTC 57.90% LOSO / 65.77% live good-mic) - per §6, the conclusion is "needs more target-age-band data," now being pursued via the data-collection game epic. Follow-up lever tried: channel-augmentation experiment + follow-ups complete 2026-07-13. Outcome: the working configuration is now a **fused two-model pair** (w2v2-CTC+channel-aug@20ep + baseline WavLM-CTC, probability-averaged) - live top-1 78.38% good mic / 52.25% bad mic, ~+8 over any single model on both conditions (channel aug hurts WavLM itself at every dose; it transforms w2v2 into the best good-mic single). Selection was done on one adult speaker's sessions - must be re-validated on child data. Full results in plans/channel-augmentation-experiment.md. Engineering readiness: fused pair ONNX-exported and numerically verified 2026-07-14 (222-clip parity replay, zero disagreements); latency is ~2x a single model and does NOT comfortably fit the ~150ms real-time budget (fine for per-clip scoring, decision on streaming use deferred to child-data validation).
**Owner:** Daniel Hair
...
- **Three-models-in-tandem analysis (user question, 2026-07-12)**, computed from the two sessions' predictions plus a probability-fusion re-score of the saved wavs: the models are genuinely complementary - at least one model is right on 83.78% of good-mic clips (60.36% bad-mic), ~18 points above WavLM alone - but no naive combiner captures it. Majority vote scores *below* WavLM alone (the two weaker models outvote it with shared errors), and averaging the three probability vectors exactly ties WavLM's top-1 (65.77% good mic; +0.9 bad mic) because the score scales aren't calibrated across model types. Only clear tandem win: fused good-mic top-3 88.29% (+4.5 over WavLM), and the target appears in *some* model's top-3 on 97.30% of good-mic clips. Conclusion: ship-shape tandem needs a learned/confidence-weighted combiner (future work, needs held-out tuning data); for now WavLM alone matches naive ensembling at a third of the inference cost.
...
### Fused-pair ONNX export + latency check (2026-07-14)

The one engineering-readiness item left open after the channel-augmentation decision is now done - `evaluation/export_fused_onnx.py` (`poe export-fused-onnx`), full numbers in plans/channel-augmentation-experiment.md and `evaluation/full_models_fused_onnx/fused_pair_onnx_report.json`:

- **The historical 0-byte CTC ONNX export failure mode is now exercised and closed for the working configuration**: all four graphs (both backbones with preprocessing baked in where the HF processor normalizes, both CTC heads) export non-empty, pass onnx.checker, and the script hard-fails otherwise.
- **Correctness verified end-to-end, not just structurally**: the pure-ONNX fused chain matches the PyTorch chain on all 222 saved live-mic clips (max fused-probability diff 4.97e-05, zero top-1 disagreements) and reproduces the pairing-eval numbers exactly (78.38%/93.69% good mic, 52.25%/70.27% bad mic).
- **Latency (the honest part): the fused pair does not comfortably fit the temporal brain's ~150ms real-time budget on a 16-core desktop CPU** - ~120-155ms at 0.5s frames (marginal), ~140-210ms at 1s, ~255-330ms for a full 3s clip; the two backbones dominate (~2x a single model, as expected). It IS comfortably fast enough for per-clip known-target scoring (the game's shape). Whether real-time streaming uses single-model WavLM with fused end-of-utterance rescoring, 0.5s hops, quantization, or a bigger budget is deferred - like the fused pair itself - to re-validation on child data.
